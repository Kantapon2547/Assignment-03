...
async function main() {
    ...
    await messageChannel.consume(queue, async (msg)=> {
        ...
        const viewedEntry = `viewed-${parsedMsg.videoId}`;
        await videosCollection.insertOne({ name: viewedEntry, videoPath: parsedMsg.videoPath }); // Record the "view" in the database.

        console.log(`Inserted ${viewedEntry} into videos collection.`);
